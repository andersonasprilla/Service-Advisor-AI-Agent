...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target_namespace:
        ...
        # ── IMPORTANT: Pass carfax_namespace to enable recall lookups ──
        ...
        # ── Handle NO_ANSWER_FOUND with language-appropriate messages ──
        if "NO_ANSWER_FOUND" in answer:
            no_answer_msgs = {
                "es": "Hmm, no encontré eso en la información que tengo. "
                      "¿Quieres que te agende una cita para que lo revise un técnico?",
                "pt": "Hmm, não encontrei isso nas informações que tenho. "
                      "Quer que eu agende uma visita para um técnico dar uma olhada?",
                "en": "Hmm, I couldn't find that in the info I have. "
                      "Want me to set up a time for you to come in and talk to one of our techs?",
            }
            msg = no_answer_msgs.get(lang, no_answer_msgs["en"])
            await update.message.reply_text(msg)
